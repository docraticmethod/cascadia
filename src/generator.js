...
export async function generateArchitecture({ action, payload }) {
  ...
  let system = `You are a software architect. Generate ARCHITECTURE.md.\n${action === 'add' ? ADD_INSTRUCTION : EDIT_INSTRUCTION}\nEvaluate whether the strategy change requires architecture changes. If not, return the existing ARCHITECTURE.md unchanged.\nReturn only the markdown content.`;
  if (templates.length) {
    system += '\n\nReference templates from the target project:\n\n' +
